...
def train_test_mtnn(train_task_csvs, test_tasks_csvs, tasks_nickname, smiles_field, y_field, id_field,
                    tempdir, no_fcn=False, gini_factor=0., num_epochs=40, batch_size=128, learning_rate=0.001, graph_conv_sizes=(128, 128),
    ...
    if op.exists(op.join(outdir, 'DONE.txt')):
        ...
        scaled_train_y, yhattrain, scaled_train_w, scaled_test_y, yhattest, scaled_test_w = \
            train_and_validate_mtnn(scaled_train, n_tasks=len(tasks), outdir=outdir, graph_conv_sizes=graph_conv_sizes,
                                    dense_size=dense_size, no_fcn=no_fcn, gini_factor=gini_factor, batch_size=batch_size, learning_rate=learning_rate,
...
def cross_validate_mtnn(task_csvs, tasks_nickname, smiles_field, split_field, y_field, id_field,
                        ...
                        dense_size=256, no_fcn=False, gini_factor=0., gpu=None):
    """
        ...
        if op.exists(op.join(outdir, 'DONE.txt')):
            ...
            train_y, yhattrain, train_w, test_y, yhattest, test_w = \
                train_and_validate_mtnn(scaled_train, len(tasks), outdir=outdir, graph_conv_sizes=graph_conv_sizes,
                                        ...
                                        no_fcn=no_fcn, gini_factor=gini_factor, num_epochs=num_epochs, pickle_file_name=tasks_nickname + '_fold_%i.pkl' % i,
...
def train_multitask_gc(train_task_csvs, tasks_nickname, smiles_field, y_field, id_field, tempdir,
                       num_epochs, no_fcn=False, gini_factor=0., batch_size=128, learning_rate=0.001, graph_conv_sizes=(128, 128),
                       dense_size=256, gpu=None):
    """
...
def dispatch_job(tasks_dir, model_nickname, tempdir_nickname, smiles_field='canonical_smiles', no_fcn=False, gini_factor=0.,
    ...
    if cv:
        ...
        return cross_validate_mtnn(files, model_nickname, split_field=split_field, smiles_field=smiles_field, no_fcn=no_fcn, gini_factor=gini_factor,
        ...
        if not final:
            ...
            return train_test_mtnn(train_files, test_files, model_nickname, smiles_field=smiles_field, no_fcn=no_fcn, gini_factor=gini_factor,
                                   y_field=y_field, id_field=id_field, tempdir=op.join(RESULTS_DIR, tempdir_nickname),
                                   num_epochs=num_epochs, batch_size=batch_size, learning_rate=learning_rate,
                                   graph_conv_sizes=graph_conv_sizes, dense_size=dense_size, gpu=gpu)
        else:
            ...
            return train_multitask_gc(train_files, model_nickname, smiles_field=smiles_field, no_fcn=no_fcn, gini_factor=gini_factor,
